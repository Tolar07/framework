...
try:
    ...
    has_binance = "#0b0e11" in body.lower() or "#0B0E11" in body
    has_old = "#0B0E13" in body and not has_binance
    print(f"\nHealth check: {url}")
    print(f"  status: {resp.status}")
    print(f"  Binance tokens: {'YES' if has_binance else 'no'}")
    print(f"  old theme: {'YES (BAD — still cached)' if has_old else 'no'}")
except Exception as e:
