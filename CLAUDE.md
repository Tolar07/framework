...
- **Frontend design-quality pack** (added 2026-08-13, presentation-layer only — never touches prediction logic, never swaps the ratified pitch-night palette without Architect ratification): `impeccable` (one-command design fixer — `/impeccable layout|typeset|polish|bolder|quieter|colorize|animate|audit|critique` etc.; full reference + detector scripts under `.claude/skills/impeccable/`), `taste-skill` (anti-slop frontend skill — three dials DESIGN_VARIANCE/MOTION_INTENSITY/VISUAL_DENSITY, pre-flight AI-tell bans), plus its sibling sub-skills `soft-skill`, `minimalist-skill`, `brutalist-skill`, `redesign-skill`, `stitch-skill`, `output-skill`, `image-to-code-skill`, `imagegen-frontend-web`, `imagegen-frontend-mobile`, `gpt-tasteskill`, `brandkit`. Source clones: `external/design-skills/impeccable/` and `external/design-skills/taste-skill/`. Use these to lift UI craft on the web dashboard and any generated marketing/landing surfaces; they are decoration/taste tooling, not prediction inputs.
