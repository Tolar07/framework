...
def test_thesportsdb_fixture_cache():
...
def test_fixtures_from_odds_derived_cache():
...
def test_api_football_plan_error_cache():
    # Isolate from the environment's plan state: the plan-error cache path is
    # Free-plan behavior (on a paid key a "plan" error is anomalous and must NOT
    # be cached). Patch is_paid_plan to False so the test asserts the Free-plan
    # path regardless of which key is in .env / plan.json.
    ...
    with _p, patch.object(fsrc, "_get_key", return_value="fake"), \
            ...
            patch.object(api_football_plan, "is_paid_plan", return_value=False):
        for _ in range(2):
            try:
...
def test_football_data_results_freshness():
    import os
...
if __name__ == "__main__":
    # Run all tests
    test_thesportsdb_fixture_cache()
    test_fixtures_from_odds_derived_cache()
    test_api_football_plan_error_cache()
    test_football_data_results_freshness()
    print("\n[OK] ALL FIXTURE CACHE TESTS PASSED")