"""
StatsArea league → URL slug mapping (PENDING live verification — HR35).

Every slug below must be empirically verified against statarea.com (HTTP 200)
before marking verified. If StatsArea changes a slug, the affected league simply
falls back to the double-outage path in booking/verify_fixtures.py (fixture
kept-but-warned), never a fabricated negative.

Format: OLP XDV league name (== config/leagues.json "name") -> StatsArea slug
which composes to https://www.statarea.com/football/<slug>/

This is the SINGLE source of truth for StatsArea league URLs.
"""

from __future__ import annotations
from typing import Dict

STATSAREA_LEAGUES: Dict[str, str] = {
    # Major European leagues (verified patterns from StatsArea)
...
BASE_URL = "https://www.statarea.com/football/{slug}/"