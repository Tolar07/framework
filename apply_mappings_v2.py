...
# Extract existing entries - handle both simple strings and tuples
# Pattern: "Key": "Value", OR "Key": ("Value", "League"),
...
# New mappings need to be formatted the same way as existing
# Check format of existing entries - they seem to use tuples for some
...
for key in sorted_keys:
    value = merged[key]
    # If value is already a tuple string, keep it; otherwise wrap in quotes
    if isinstance(value, str) and value.startswith("("):
        new_dict_lines.append(f'    "{key}": {value},')
    else:
        new_dict_lines.append(f'    "{key}": "{value}",')
