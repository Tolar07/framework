...
def _load_predictz_map():
    fallback = ({"Premier League": "england/premier-league"},
                "https://www.predictz.com/football/{slug}/")
    try:
        spec = importlib.util.spec_from_file_location(
            "predictz_leagues", _REPO_ROOT / "config" / "predictz_leagues.py")
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.PREDICTZ_LEAGUES, mod.BASE_URL
    except Exception:
        return fallback

PREDICTZ_LEAGUES, BASE_URL = _load_predictz_map()
