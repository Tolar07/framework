...
**OLP XDV note (Windows, installed 2026-08-11):** `sports-skills` is installed
in this repo's environment. The `sports-skills` console script is NOT on PATH in
this shell — invoke it as `python -m sports_skills` (or `py -3.12 -m sports_skills`).
Source lives at `external/sports-skills` (repo root, sibling to `olp_xdv_agent/`).

Before first use, check if the CLI is available:
```bash
python -m sports_skills --help
```
If the package is missing (returns `ModuleNotFoundError`), install from the local
clone (no network needed):
```bash
pip install -e "/c/Users/Motunrayo/omniroute test/external/sports-skills"
```
...
```bash
python -m sports_skills football get_daily_schedule
python -m sports_skills football get_season_standings --season_id=premier-league-2026
```
